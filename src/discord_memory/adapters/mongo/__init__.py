...
class MongoStore:
    """Implements :class:`~discord_memory.ports.MemoryStore` on MongoDB."""

    def __init__(self, url: str, database: str = "discord_memory") -> None:
        try:
            from pymongo import AsyncMongoClient
        except ImportError as exc:  # pragma: no cover - requires the extra
            raise ImportError(
                "the MongoDB adapter requires pymongo>=4.10 (pip install discord-memory[mongo])",
            ) from exc
        ...
        from discord_memory.adapters.mongo.vectors import MongoVectorIndex
