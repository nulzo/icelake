...
from icelake.retrieval.injection import InjectionBuilder, estimate_tokens
...
def _build_llm(
    ...
    *,
    ...
    from icelake.adapters.llm_openai_compat import build_chat_llm
...
def _build_store(config: MemoryConfig) -> MemoryStore:
    ...
    if backend == "mongo":
        ...
        database = url.rsplit("/", 1)[-1].split("?")[0] or "icelake"
    ...
    raise ConfigError(
        ...
        "(e.g. pip install icelake[mongo]) or a store override",
    )
